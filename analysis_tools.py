...
def get_df_from_ML_optimization_Ni_Cr(path_suggested_exps, 
                                path_input_params, 
                                path_goal_params):

    # Creating a df for each run with Jonas 

    timestamp_to_beta = {}

    for file in os.listdir(path_suggested_exps):
        if file.endswith(".json"):
            if "suggested" in file:
                with open(os.path.join(path_suggested_exps, file), "r") as f:
                    suggested_exp = json.load(f)
                    timestamp = list(suggested_exp.keys())[0]
                    timestamp_key = list(suggested_exp.keys())[-1]
                    ...
                    timestamp_to_beta[formatted_str] = beta
                    #print(suggested_exp.keys())

    def find_beta_near(dt_query, tolerance_sec=60):
        for dt_key in timestamp_beta_dt:
            if abs((dt_key - dt_query).total_seconds()) <= tolerance_sec:
                return timestamp_beta_dt[dt_key]
        return 0

    timestamp_beta_dt = {}
    for ts_str, beta in timestamp_to_beta.items():
        dt = datetime.strptime(ts_str, "%d.%m.%Y_%H-%M")
        timestamp_beta_dt[dt] = beta
    ...
    with open(path_goal_params, "r") as f:
        goal_params = json.load(f)
        print(goal_params, "These are goal params")
    # Create a list of dictionaries (rows)
    data_rows = []

    # Iterate through matching keys in both input and goal parameter sets
    for experiment_i_goals, experiment_i_inp in zip(goal_params, input_params):
        try:
            #time_format = start_time_02.05.2025 at 11:45"
            #print(experiment_i_goals)
            timestamp_str_exp_i = experiment_i_goals.split("start_time_")[-1].split("_")[0]
            dt = datetime.strptime(timestamp_str_exp_i, "%d.%m.%Y at %H:%M")
            formatted = dt.strftime("%d.%m.%Y_%H-%M")
            ...
            if beta_value == 0:
                ML_optimization = "random distribution"
                print(formatted)
            else:
                ML_optimization = "qUpperConfidenceBound"
            
            print(goal_params[experiment_i_goals])
            features = {
                "NiSO4 (mol/L)": input_params[experiment_i_inp]["Conc NiSO4"],
                "CrCl3 (mol/L)": input_params[experiment_i_inp]["Conc CrCl3"],
                "H2SO4 (mol/L)": input_params[experiment_i_inp]["Conc H2SO4"],
                "Dep t (s)": input_params[experiment_i_inp]["Dep time [s]"],
                "Dep I (mA/cm²)": input_params[experiment_i_inp]["Current density mA/cm2"],
                "Dep T (C)": input_params[experiment_i_inp]["Dep electrolye T [C]"],
                "integrated_area": goal_params[experiment_i_goals]["Integrated stability at 10 [mA/cm2]"],
                "ML_optimization" : ML_optimization,
                "timestamp" : formatted,
                "qUpperConfidence_beta" : beta_value
            }
            data_rows.append(features)

        except KeyError as e:
            print(f"Skipping {experiment_i_inp} due to missing key: {e}")
